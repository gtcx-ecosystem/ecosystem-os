---
title: 'Guide: Adding a New Service'
status: current
date: 2026-05-24
...
| `03-platform/packages/` | Shared libraries consumed by apps and services   | `03-platform/packages/config`, `03-platform/packages/shared-types`, `03-platform/packages/utils` |
...
4. **Is it a reusable library with no runtime of its own?** Put it in `03-platform/packages/`.

If you are unsure, default to `services/` for anything with a runtime and `03-platform/packages/` for anything without.
...
```json
{
  ...
  "scripts": {
    "build": "tsc",
    "dev": "tsx watch 03-platform/src/main.ts",
    "lint": "eslint 03-platform/src/",
...
```json
{
  "extends": "../../03-platform/packages/config/tsconfig.base.json",
  "compilerOptions": {
    "outDir": "dist",
    "rootDir": "src"
  },
...
```bash
SERVICE_GROUP="\<group\>"  # e.g., "products", "engines", "infrastructure"
SERVICE_NAME="\<service-name\>"

mkdir -p "01-docs/specs/${SERVICE_GROUP}/${SERVICE_NAME}"/{01_context,02_vision,03_product,04_experience,05_specs,06_planning,07_backend,08_frontend,09_security,10_internal}
```
...
| Folder                           | Contents                               |
| -------------------------------- | -------------------------------------- |
| `01_context/`       | Domain context and stakeholders        |
| `02_vision/`         | Service vision and strategic alignment |
| `03_product/`       | Product requirements and user stories  |
| `04_experience/` | UX flows, wireframes, design specs     |
| `05_specs/`           | Technical specs and ADRs               |
| `06_planning/`     | Roadmap, sprint plans, milestones      |
| `07_backend/`       | API design, data models, integrations  |
| `08_frontend/`     | UI components, state, routing          |
| `09_security/`     | Threat models, auth, compliance        |
| `10_internal/`     | Internal notes and scratch docs        |
```
...
- [1-protocols/code-standards/protocol.md](../../../governance/protocols/5-code-standards/protocol.md)
- [../../reference/templates/agents/1-onboarding/service-overview.md](../../agent-sops/1-onboarding/service-overview.md)
