---
title: 'Guide: New Repository Setup'
status: 'current'
date: '2026-05-24'
...
- [ ] GitHub org access (`[org-name]` or the relevant org)
...
Choose a name that follows [naming conventions](../../../governance/protocols/3-naming-conventions/protocol.md): lowercase, kebab-case, descriptive.
...
Create `.eslintrc.json` matching [code standards protocol](../../../governance/protocols/5-code-standards/protocol.md).
...
```bash
mkdir -p 01-docs/{architecture/{decisions,data-models,deployment,monitoring,principles,global-south},specs/{_project/{overview,planning},products,engines,infrastructure},engineering/{guides,security,testing,devops,data},operations/{organization,runbooks,compliance,analytics,accessibility},reference/{archived,research,glossary,changelog,legal},gitbook}
```

Copy the docs README template and customize it for your repo:

```bash
cp readme/templates/docs-folder.md 01-docs/README.md
```

Add a README to each root folder and `.gitkeep` to empty folders so the structure is visible in git.

See [1-protocols/docs-structure/protocol.md](../../../governance/protocols/1-docs-structure/protocol.md) for the full standard.
...
Create shared config packages in `03-platform/packages/config/` for shared tsconfigs and ESLint configs.
...
- [ ] CI pipeline passes
- [ ] `01-docs/` structure exists with populated README
...
- [1-protocols/repo-setup/protocol.md](../../../governance/protocols/2-repo-setup/protocol.md)
- [GitHub README template](../../../engineering/system-design/github-readme-template.md)
