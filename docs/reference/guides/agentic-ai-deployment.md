...
- **VIA mobile education app specifics**: Multilingual lesson delivery, offline learning, and user onboarding are detailed in [VIA (Virtual Instructor Agent)](via-virtual-instructor-agent.md)
- **VXA field inspection and verification**: Evidence capture, inspection checklists, and compliance attestation are covered in [VXA (Virtual Inspection Agent)](vxa-tm-virtual-inspection-agent.md)
- **General VIA/VXA architecture overview**: System components, GTCX integration, and performance metrics are described in [VIA/VXA Overview](via-vxa-overview.md)
