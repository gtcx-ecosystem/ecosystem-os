  ...
  for (const init of inits) {
    ...
    if (children.length === 0) {
      gaps.push({
        ...
        title: init.title ?? id,
        reason: 'no_stories',
        fix: `Add docs/product/roadmap/stories/STORY-*.md with initiativeId: ${id} → pnpm product:compile`,
      });
      ...
      gaps.push({
        ...
        fix: `Open or add a pending story under initiative ${id} → product:compile`,
      });
    }
  }
  return gaps;
...
export function writeOperationsBacklogMirror(repoRoot, backlogDoc) {
  const target = join(repoRoot, 'ops/machine/backlog.json');
  mkdirSync(join(repoRoot, 'ops/machine'), { recursive: true });
  const mirror = {
    ...backlogDoc,
    _mirror: {
